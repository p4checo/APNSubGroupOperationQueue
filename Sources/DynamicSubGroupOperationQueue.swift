...
/// `DynamicSubGroupOperationQueue` is an `OperationQueue` subclass which allows scheduling operations in serial
/// subgroups inside a concurrent queue.
///
...
public final class DynamicSubGroupOperationQueue: OperationQueue {
    ...
    ///
    /// - Parameters:
    ///   - op: The `NSOperation` to be added to the queue.
    ...
    ///
    /// - Parameters:
    ///   - ops: The `[NSOperation]` to be added to the queue
    ///   - key: The subgroup's identifier key
    ...
    /// - Returns: An `[NSOperation]` containing a snapshot of all currently scheduled (non-finished) subgroup
    /// operations.
